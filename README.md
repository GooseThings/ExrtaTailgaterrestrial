...
hardware will be in the Ku band (~11ghz)
 ...
 - Added 9/11/2026 - dish_web.py is a browser-based control panel that combines scanning, image viewing, and tracking into one page.
...
"seismic" and "gnuplot2" also work well, though I feel they lose some definition on the background
...
of this file. However, I may have to refer back to this myself to remember how it works! 
 ...
 # dish_web.py
 A browser-based control panel that combines scanning, image viewing, and satellite tracking
 into a single page, so you don't have to juggle three separate scripts.
...
```
 ...
 - Run ```python3 dish_web.py```
 - Open a browser to http://localhost:5000
 - Enter your serial port (default /dev/ttyACM0) and click "Connect" — this connection is shared
   across all three tabs
 - **Scan tab:** set your azimuth/elevation range and resolution, check the time estimate, then
   click "Start Scan". A live preview and log update as the scan runs. Files are saved with the
   same naming convention as dish_scan.py, so they show up in the View tab automatically.
 - **View Image tab:** pick a saved scan file, adjust the interpolation factor if you like, and
   click "Render" to see the heatmap.
 - **Track tab:** click "Load Satellites" to pull the current active-satellite catalogue from
   Celestrak, select one, and click "Start Tracking".

 This is a local tool — the Flask dev server only listens on localhost and isn't meant to be
